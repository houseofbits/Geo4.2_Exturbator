...
namespace OGLFT {
  ...
  void Face::draw ( GLfloat x, GLfloat y, const char* s )
  {
	...
	break;
      ...
      glRotatef( string_rotation_, 0.0f, 0.0f, 1.0f );
  ...
  void Monochrome::renderGlyph ( FT_Face face, FT_UInt glyph_index )
  {
    ...
    glBitmap( bitmap_glyph->bitmap.width, bitmap_glyph->bitmap.rows,
	      ...
	      (float)bitmap_glyph->bitmap.rows - (float)bitmap_glyph->top,
  ...
  void Grayscale::renderGlyph ( FT_Face face, FT_UInt glyph_index )
  {
    ...
    glBitmap( 0, 0, 0, 0,
	      (float)bitmap_glyph->left,
	      (float)bitmap_glyph->top - (float)bitmap_glyph->bitmap.rows,
	      0 );
    ...
    glBitmap( 0, 0, 0, 0,
	      -(float)bitmap_glyph->left + face->glyph->advance.x / 64.0f,
		(float)bitmap_glyph->bitmap.rows - (float)bitmap_glyph->top +
	      face->glyph->advance.y / 64.0f,
	      0 );
  ...
  void Translucent::renderGlyph ( FT_Face face, FT_UInt glyph_index )
  {
    ...
    glBitmap( 0, 0, 0, 0,
	      -(float)bitmap_glyph->left + (float)face->glyph->advance.x / 64.0f,
		(float)bitmap_glyph->bitmap.rows - (float)bitmap_glyph->top +
	      face->glyph->advance.y / 64.0f,
	      0 );
  ...
  void Outline::renderGlyph ( FT_Face face, FT_UInt glyph_index )
  {
    ...
    vector_scale_ = ( point_size_ * resolution_ ) / ( 72.0f * face->units_per_EM );

    if ( character_rotation_.active_ ) {
      glPushMatrix();
      glTranslatef( ( face->glyph->metrics.width / 2.0f +
		      face->glyph->metrics.horiBearingX ) / 64.0f
		    * (float)vector_scale_,
		    rotation_offset_y_,
		    0.0f);

      if ( character_rotation_.x_ != 0.0f)
	glRotatef( character_rotation_.x_, 1.0f, 0.0f, 0.0f);

      if ( character_rotation_.y_ != 0.0f)
	glRotatef( character_rotation_.y_, 0.0f, 1.0f, 0.0f);

      if ( character_rotation_.z_ != 0.0f)
	glRotatef( character_rotation_.z_, 0.0f, 0.0f, 1.0f);
    ...
    glTranslatef( face->glyph->advance.x / 64.0f * (float)vector_scale_,
		  face->glyph->advance.y / 64.0f * (float)vector_scale_,
		  0. );
  ...
  void Filled::renderGlyph ( FT_Face face, FT_UInt glyph_index )
  {
    ...
    if ( depth_offset_ != 0.0f) {
      glPushMatrix();
      glTranslatef( 0.0f, 0.0f, depth_offset_ );
  ...
  void Texture::renderGlyph ( FT_Face face, FT_UInt glyph_index )
  {
    ...
    if ( character_rotation_.active_ ) {
      glPushMatrix();
      glTranslatef( ( texture_info.width_ / 2.0f +
		      texture_info.left_bearing_ ),
		    rotation_offset_y_, 0.0f);

      if ( character_rotation_.x_ != 0.0f)
	glRotatef( character_rotation_.x_, 1., 0.0f, 0.0f);
    ...
    glTexCoord2i( 0, 0 );
    glVertex2f((float)texture_info.left_bearing_, (float)texture_info.bottom_bearing_ );

    glTexCoord2f( texture_info.texture_s_, 0.0f);
    glVertex2f((float)texture_info.left_bearing_ + (float)texture_info.width_,
		(float)texture_info.bottom_bearing_ );

    glTexCoord2f( texture_info.texture_s_, texture_info.texture_t_ );
    glVertex2f((float)texture_info.left_bearing_ + (float)texture_info.width_,
		(float)texture_info.bottom_bearing_ + (float)texture_info.height_ );

    glTexCoord2f( 0.0f, (float)texture_info.texture_t_ );
    glVertex2f((float)texture_info.left_bearing_,
		(float)texture_info.bottom_bearing_ + (float)texture_info.height_ );
    ...
    // Drawing a character always advances the MODELVIEW.
    glTranslatef( texture_info.advance_.x / 64.0f,
		  texture_info.advance_.y / 64.0f,
		  0.0f);
  }
  ...
  void MonochromeTexture::bindTexture ( FT_Face face, FT_UInt glyph_index )
  {
    ...
    texture_info.bottom_bearing_ = -( (int)face->glyph->bitmap.rows - (int)face->glyph->bitmap_top );
