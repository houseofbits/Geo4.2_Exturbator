...
//Font system_font = Font();

Font Font::m_DefaultInstance;
...
{
//	PFSFile* f = new PFSFile(file_name, PFSFile::F_READ);
	/**
	FILE *f = 0;
	...
	if(f){
		...
		//f->Read((char *)&header, sizeof(header), 1);
		fread((char *)&header, sizeof(header), 1, f);
		header.tex = tex;
		...
		//f->Read((char *)header.chars, sizeof(GLFontChar) * num_chars, 1);
		fread((char *)header.chars, sizeof(GLFontChar) * num_chars, 1, f);

		//Read texture pixel data
		num_tex_bytes = header.tex_width * header.tex_height * 2;
		tex_bytes = new char[num_tex_bytes];
		//f->Read(tex_bytes, num_tex_bytes, 1);
		fread(tex_bytes, num_tex_bytes, 1, f);
	...
	//cout << tex << endl;
	//cout << header.tex_width << endl;
	//cout << header.tex_height << endl;
